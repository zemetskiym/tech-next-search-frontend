import './App.css';
import logo from "./logo.png"
import { useState } from 'react';

function App() {
    const [input, setInput] = useState("");

    function handleSubmit(e) {
        e.preventDefault() 
    };

    function handleInput(e) {
        if (e.target.value != "ENTER") {setInput(e.target.value)}
    }

    return (
    <header className="header">
        <nav className="nav">
            <img src={logo} width={375} height={66} />
            <a className='link'>Backend source code</a> 
            <a className='link'>Frontend source code</a>
            <a className='link'>Data visualization</a>
        </nav>
        <section className='search'>
            <form onSubmit={(e) => handleSubmit(e)}>
                <input
                    type="text"
                    value={input}
                    onChange={(e) => handleInput(e)}
                    className='searchbar'
                />
                <div className='icon'>
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="#979797" d="m19.6 21l-6.3-6.3q-.75.6-1.725.95T9.5 16q-2.725 0-4.612-1.888T3 9.5q0-2.725 1.888-4.612T9.5 3q2.725 0 4.612 1.888T16 9.5q0 1.1-.35 2.075T14.7 13.3l6.3 6.3l-1.4 1.4ZM9.5 14q1.875 0 3.188-1.313T14 9.5q0-1.875-1.313-3.188T9.5 5Q7.625 5 6.312 6.313T5 9.5q0 1.875 1.313 3.188T9.5 14Z"/></svg>
                </div>
            </form>
        </section>
    </header>
    );
}

export default App;
